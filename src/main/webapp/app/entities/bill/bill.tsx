...
export const Bill = (props: IBillProps) => {
  ...
  const { billList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="bill-heading">
        Bills
          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('isPayed')}>
                  Is Payed <FontAwesomeIcon icon="sort" />
                </th>
            ...
            <tbody>
              {billList.map((bill, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{bill.balance}</td>
                  <td>{bill.isPayed ? 'true' : 'false'}</td>
                  <td>{bill.source}</td>
                  <td>{bill.checkIn ? <Link to={`check-in/${bill.checkIn.id}`}>{bill.checkIn.id}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${bill.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${bill.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
